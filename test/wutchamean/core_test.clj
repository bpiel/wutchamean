...
(fact "match token"
      ...
      => [{:confidence 0.6666666666666667, :match "hunted", :matches nil}
          {:confidence 0.6666666666666667, :match "hunter", :matches nil}]
      )

(fact "tokenize"
      (tokenize (process-grammar (:grammar grammar)) "Dog cat wumpus hutne")
      => {0
          [{:matches [{:confidence 1.0, :match "dog", :matches nil}],
            :original "Dog",
            :position 0}],
          1
          [{:matches
            [{:confidence 1.0,
              :match "cat",
              :matches ["cat" {:class :animal, :index 0, :string "cat"}]}],
            :original "cat",
            :position 1}],
          2
          [{:matches [{:confidence 1.0, :match "wumpus", :matches nil}],
            :original "wumpus",
            :position 2}],
          3
          [{:matches
